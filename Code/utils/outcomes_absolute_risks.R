get_absolute_risks <- function(data,
  ...
  for (i in seq_along(event_vars)) {
    ...
    df_absolute_risks <- compute_absolute_risks(
      data = data,
    ...
    if (bootstrap) {
      df_absolute_risks_CI <- risks_boots(
        data = data,
        model_PS = model_PS,
        event_var = event_var,
        competing_event_var = competing_event_var,
      ...
      df_absolute_risks <- df_absolute_risks |>
        dplyr::bind_rows(df_absolute_risks_CI)
    }
    ...
    df_absolute_risks <- df_absolute_risks |>
      ...
      tidyr::pivot_wider(
        names_from = name,
        values_from = c(A0, A1, rd, rr, rmst0, rmst1, rmst_diff)
      )
...
compute_absolute_risks <- function(data,
                                   elig_cohort,
                                   event_var,
                                   competing_event_var = "none",
                                   time2event_var,
  ...
  # Compute risk differences, risk ratios and RMST
  dat.table <- dat.table |>
    dplyr::mutate(
      rd = R1 - R0,
      rr = R1 / R0,
