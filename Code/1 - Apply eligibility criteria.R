...
egfr_dt <- filled_eGFR[!is.na(egfr2021), c("LOPNR", 
                                           "visit_date",
                                           "birthdate", 
                                           "DODSDAT",
                                           "egfr2021")]
...
low_egfr <- egfr_dt[egfr2021 < 20, c("LOPNR",
                                     "visit_date",
                                     "next_visit",
                                     "birthdate", 
                                     "DODSDAT")]
...
low_egfr[, elig_until_1 := eligibility_end(visit_date = visit_date,
...
age_dt <- merged_ckd_bday80[low_egfr[, -c("DODSDAT")], 
                            on = c("LOPNR", "visit_date")]
...
age_above_80[, elig_until_2 := eligibility_end(visit_date = visit_date,
                                               next_visit = next_visit,
                                               date_of_death = DODSDAT)]
elig_above_80 <- age_above_80[, c("LOPNR",
                                  "visit_date",
                                  "birthdate",
                                  "DODSDAT",
                                  "female",
...
max_date_dict <- unique(
  merged_ckd_bday80[
    (LOPNR %in% IDs_65_80) &
      (is.na(DODSDAT) | DODSDAT > bday80),
    .(LOPNR, max_date = pmin(end_date, bday80))
  ]
)
...
eligible_visits <- new_visit_dates[age_65_80, 
                                   on = .(LOPNR,
...
new_rows <- new_visit_dates_range[age >= 65 & age < 80 &
                                    (visit_date < DODSDAT | is.na(DODSDAT))]
...
age_65_80_full <- rbindlist(list(age_65_80_comorb[, c("LOPNR",
                                                      "visit_date",
                                                      "decision_date1",
                                                      "decision_modality1",
                                                      "birthdate",
...
for (comorbidity in Davies_comorbidities) {
  age_65_80_comorbidities <- retrieve_past_info(
    ...
    max_roll_days = ifelse(comorbidity == "cancer", 3 * 365.25, Inf),
...
elig_65_80_Davies[, elig_until_2 := eligibility_end(visit_date = visit_date,
                                                    next_visit = next_visit,
                                                    date_of_death = DODSDAT)]
...
elig_65_80_Davies[, elig_until_2 := pmin(
  elig_until_2,       # original eligibility
  first_80_visit - 1, # cannot exceed first 80+ visit
  na.rm = TRUE
)]

# 6. combine data where age >= 65 and Davies >=2 OR age >= 80
age_crit_dt <- rbindlist(list(elig_65_80_Davies[, c("LOPNR",
                                                    "visit_date",
                                                    "birthdate",
                                                    "DODSDAT",
                                                    "female",
                                                    ...
                                                    ..Davies_comorbidities)], 
                              elig_above_80),
                         fill=TRUE) # elig_above_80 does not have comorbidities yet
...
first_dia_trans <- merged_ckd[!is.na(krt_startdate) |
                                ...
                                .(date_dia_trans = min(krt_startdate, 
                                                       decision_tx_date1,
                                                       decision_date1+1, 
                                                       na.rm = TRUE)), 
                                   by = "LOPNR"]
...
hiv_dem <- hiv_dementia[LOPNR %in% no_trans_dia_dt$LOPNR &
                          (hiv == 1 | dementia == 1)]
first_hiv_dem <- hiv_dem[, .(date_hiv_dementia = min(visit_date)), 
                         by = "LOPNR"]
...
# Calculate Davies score
age_80_comorbidities[, Davies_score := rowSums(.SD, na.rm = TRUE), 
                     .SDcols = Davies_comorbidities]
...
setDT(outpatient)
first_palliative <- outpatient[
  LOPNR %in% age_80_comorbidities$LOPNR & 
    (MVO == "061" | MVO == "020" | MVO == "243" | MVO == "246"),
  c("LOPNR", "INDATUMA"), 
  with = FALSE
][, `:=`
  (INDATUMA = as.IDate(as.character(INDATUMA), format = "%Y%m%d"))
][
  order(INDATUMA),
  .SD[1],
  by = "LOPNR"
...
elig_dt_expanded <- no_palliative_dt[, .(date = seq(visit_date,
                                                    elig_until_6,
                                                    by = "day")), 
                                         by = c("LOPNR",
                                                "visit_date",
