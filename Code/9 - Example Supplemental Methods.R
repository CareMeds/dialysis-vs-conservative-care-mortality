...
### fit risk model
################################################################################
elig_Surv <- survival::Surv(elig_cohort$time2event_death_2y, 
                            elig_cohort$event_death_2y)
...
################################################################################
baseline$lp_risk <- predict(risk_model,
                            newdata = baseline)
# baseline$pred_risk <- PredictionTools::fun.event(h0 = h0_risk,
#                                                  lp = baseline$lp_risk)
...
cat("Patient is", baseline[nr_patient, ][["age"]], "years old \n",
    "eGFR                                 :", baseline[nr_patient, ][["egfr2021"]], "ml/min/m3 \n",
    "Albumin                              :", baseline[nr_patient, ][["albumin"]], "g/L \n",
    "Coefficients risk model              :", round(coef_risk, 4), "\n",
    "Centers risk model                   :", round(centers_risk, 4), "\n",
    "Baseline hazard at two years for risk:", round(h0_risk, 4), "\n", 
    "Linear predictor value               :", round(manual_lp_risk, 4), "\n",
    "Probability risk                     :", round(manual_prob_risk, 4), "\n",
    "Baseline hazard at two years for ITE :", round(h0_ITE, 4), "\n",
    "Coefficients ITE model               :", round(coef_ITE, 4), "\n",
    "Centers ITE model                    :", round(centers_ITE, 4), "\n",
    "Linear predictor under dialysis      :", round(manual_lp_ITE, 4), "\n",
    "Linear predictor under CC            :", round(manual_lp_ITE_counterfactual, 4), "\n",
    "Probability risk under dialysis      :", round(manual_prob_ITE, 4), "\n",
    "Probability risk under CC            :", round(manual_prob_ITE_counterfactual, 4), "\n",
    "Risk difference                      :", round(RD, 4), "\n",
    "RMST under dialysis                  :", round(rmst_1, 1), "\n",
    "RMST under CC                        :", round(rmst_0, 1), "\n",
    "Difference in RMST                   :", round(rmst_1 - rmst_0, 1), "\n",
